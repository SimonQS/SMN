...
from utils import load_mag, sgc_precompute, set_seed, smn_precompute, sub_cs, sub_topk
...
adj, features, labels, idx_train, idx_val, idx_test = load_mag(args.dataset, 
                                                            args.normalization, 
                                                            cuda = True)
...
if args.model == "SMN" or args.model == "SGC":
    ...
    # Community Search 
    query_nodes = torch.multinomial(idx_test.type(torch.FloatTensor).cuda(), 50, replacement = True)#.tolist()
    query_labels = labels[query_nodes].round().type(torch.LongTensor)
    ...
    if args.case == 1:
        ...
        if args.cs == 'sub_cs':
            communities, cs_time = sub_cs(adj, emb, query_nodes, ssf, 
                            community_size = 30, early_stop = 16, lp_filter = args.ssf)
            print('test', args.ssf)
        if args.cs == 'sub_topk':
            communities, cs_time = sub_topk(adj, emb, query_nodes, ssf, 
                            community_size = 30, early_stop = 16, lp_filter = args.ssf)
